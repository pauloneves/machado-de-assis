...
@pytest.fixture
def livro():
    parse.get_livro("livros/Papéis avulsos_files/tx_Papeisavulsos.html")
...
def test_ajusta_titulo_conto(livro):
    b = book(
        """<!-- *********************************** O EMPRESTIMO  ****************************************************** -->
...
"""
    ...
    parse.ajusta_titulos(b)
    assert b.h2.text.lower() == "o empréstimo"
...
def test_ajusta_referencia():
    ...
    assert '<a href="#mynewanchorNTA1">texto</a>' == str(ref)
...
def test_append_notas(livro):
    ...
    h2 = livro.find("h2")
    assert h2
    assert len(h2.find_all("p")) == len(notas)


def test_reorganiza_notas(livro):
    parse.ajusta_titulos(livro)
    parse.reorganiza_notas(livro)
